...
namespace Pannexus.PsNutrac.Migrations.SeedData
{
    public class TenantRoleAndUserBuilder
    {
        ...
        private void CreateRolesAndUsers()
        {
            ...
            if (userRole == null)
            {
                userRole = _context.Roles.Add(new Role(_tenantId, StaticRoleNames.Tenants.User, StaticRoleNames.Tenants.User) { IsStatic = true, IsDefault = true });
                _context.SaveChanges();

                //Grant user permissions to user role
                var permissions = PermissionFinder
                    .GetAllPermissions(new SampleLTEAuthorizationProvider())
                    .Where(p => p.MultiTenancySides.HasFlag(MultiTenancySides.Tenant) && !p.Name.Contains("Admin"))
                    .ToList();
